...
public abstract class Feature
   ...
   * Earth List View, each Feature has a checkbox that allows the user to
   * control visibility of the Feature.
   * </p>
   * <p>
   * <p>
   * <p>
   ...
   * specification is found at http://atompub.org. (see the sample that
   * follows).
   * </p>
   * <p>
   * These elements are defined in the Atom Syndication Format. The complete
   ...
   * is not supplied, the first two lines of the <description> are used. In
   ...
   * not support HTML markup. <Snippet> has a maxLines attribute, an integer
   ...
   * below, followed by information common to both.
   * </p>
   * <p>
   ...
   * supported content for the <description> element changed from Google Earth
   * 4.3 to 5.0. Specific information for each version is listed out below,
   * followed by information common to both. Google Earth 5.0 Google Earth 5.0
   * supports plain text content, as well as full HTML and JavaScript, within
   * description balloons. Contents of the description tag are rendered by the
   * WebKit open source web browser engine, and are displayed as they would be
   * in any WebKit-based browser. General restrictions Links to local files are
   * generally not allowed. This prevents malicious code from damaging your
   * system or accessing your data. Should you wish to allow access to your
   ...
   * are always allowed, if contained within an <img> tag. Content that has been
   * compressed into a KMZ file can be accessed, even if on the local
   ...
   * drawn only when the Region is active. When the <viewRefreshMode>
   * is onRegion, the Link or Icon is loaded only when the Region is active. See
   * the "Topics in KML" page on Regions for more details. In a Container or
   ...
   * typed data. A given KML Feature can contain a combination of these types of
   * custom data.
   * </p>
   * <p>
   ...
   * </p>
   * <p>
   * The ExtendedData element offers three techniques for adding custom data to
   * a KML Feature (NetworkLink, Placemark, GroundOverlay, PhotoOverlay,
   ...
   * @see timePrimitive
   *
   * @return possible object is null   {@code <}{@link TimeSpan}{@code>}
   ...
   * @see timePrimitive
   *
   * @param value allowed object is null   {@code <}{@link TimeSpan}{@code>}
     *     {@code <}{@link TimeStamp}{@code>}
   *     {@code <}{@link TimePrimitive}{@code>}
   *
   */
