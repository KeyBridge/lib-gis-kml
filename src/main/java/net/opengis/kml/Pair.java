...
public class Pair
   ...
   * style is in the same file, use a # reference. If the style is defined in an
   * external file, use a full URL along with # referencing. Examples are
   ...
   * styles derived from &lt;ColorStyle&gt;.) A style defined within a Feature
   * is called an "inline style" and applies only to the Feature that contains
   ...
   * referenced by one or more Features within the &lt;Document&gt;. In cases
   * where a style element is defined both in a shared style and in an inline
   * style for a Feature—that is, a Folder, GroundOverlay, NetworkLink,
   * Placemark, or ScreenOverlay—the value for the Feature's inline style takes
   * precedence over the value for the shared style. Syntax:
